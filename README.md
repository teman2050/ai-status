...
- **Codex** — connected automatically: the app points `notify` in `~/.codex/config.toml` at itself (`--hook codex`). Codex has a single notify slot, so an existing notifier is preserved by chaining — its command is kept after `--chain` and re-invoked on every notification — and restored exactly when the panel's "Auto-connect Codex" toggle is turned off. Turn logs are polled automatically either way, including the newer `payload.id` session field and activity events emitted after recent Codex updates. For development, `adapters/codex/asb_notify_chain.sh` can still be wired manually.
- **Cursor** — connected automatically via `~/.cursor/hooks.json` (`--hook cursor`), again only ever touching its own entries. Only observational events plus `beforeSubmitPrompt` are registered; gating events are answered with pass-through decisions so Cursor is never blocked. The panel's "Auto-connect Cursor" toggle removes the entries again. For development, `adapters/cursor/asb_cursor_hook.py` can still be registered manually.
