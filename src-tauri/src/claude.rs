use crate::store::{quota_reset_passed, transcript_limit, Blocked, IncomingEvent, Store};
...
fn scan_blocked(base: &Path) -> HashMap<String, ClaudeQuota> {
    ...
    for path in recent_transcripts(base) {
        ...
        // a limit whose stated reset moment already passed is over — the tail just never
        // updated because the block froze the session; re-flagging it would resurrect the
        // countdown past its own reset (and fight the store's expiry release every 3s)
        if quota_reset_passed(
...
mod tests {
    ...
    #[test]
    fn scan_blocked_skips_a_limit_whose_reset_already_passed() {
        use chrono::{DateTime, Local};
        let mut dir = std::env::temp_dir();
        dir.push(format!("asb_claude_expired_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let path = dir.join("session-b.jsonl");
        // limit reported 6h ago stating a reset 5h ago -> over; must not resurrect the pause
        ...
        writeln!(
            f,
            r#"{{"sessionId":"session-b","cwd":"C:\\work\\LongTodo","isApiErrorMessage":true,"message":"Usage limit reached. Resets at {}"}}"#,
            stated.format("%H:%M"),
        )
        .unwrap();
        drop(f);
        ...
        assert!(scan_blocked(&dir).is_empty());
        let _ = fs::remove_dir_all(&dir);
    }
}
